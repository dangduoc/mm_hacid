...
namespace BaseProjectWebRazor.Areas.Admin.Pages.About.AboutProject
{
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            Projects = (await _context.AboutProject.ToListAsync()).OrderByDescending(c => c.Index)
                        .Select(c => new AboutProjectDto
                        {
                            Id = c.Id,
                            TabTitle = c.TabTitle,
                            Summary = c.Summary,
        ...
        public async Task<IActionResult> OnDeleteAsync(int Id)
        {
            ...
            _context.AboutProject.Remove(entity);
        ...
        public async Task<IActionResult> OnPostChangeIndex(int Id, bool moveUp = true)
        {
            ...
            domain.AboutProject swapNew = null;
            //up
            if (moveUp)
            {
                swapNew = await _context.AboutProject.Where(c => c.Index > entity.Index).OrderBy(c => c.Index).FirstOrDefaultAsync();
            }
            else
            {
                swapNew = await _context.AboutProject.Where(c => c.Index < entity.Index).OrderByDescending(c => c.Index).FirstOrDefaultAsync();
            }
            ...
            _context.AboutProject.Update(entity);
            _context.AboutProject.Update(swapNew);
